...
import { Bell, Send, Trash2, RefreshCw, CheckCheck, Reply } from 'lucide-react'
...
export default function ManagerNotifications() {
  ...
  const [replyTarget, setReplyTarget] = useState(null)   // the notification being replied to
  ...
  const handleSend = async () => {
    ...
    setSending(true)
    try {
      const user_ids = form.user_ids.length > 0
        ? form.user_ids
        : users.map(u => u._id)

      if (user_ids.length === 0) {
        toast.error('No users available to send to')
        setSending(false)
        return
      }
  ...
  const handleReply = async () => {
    ...
    // Determine who to reply to: prefer notif.sender_id, fall back to notif.user_id
    const recipientId = replyTarget?.sender_id || replyTarget?.user_id
    if (!recipientId) {
      toast.error('Cannot determine reply recipient')
      return
    }
  ...
  const selectAll = () =>
    setForm(f => ({
      ...f,
      user_ids: f.user_ids.length === users.length ? [] : users.map(u => u._id),
    }))
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-2">
        {loading ? (
          ...
          <EmptyState icon={Bell} title="No notifications" description="Send a notification to your team" />
        ) : notifs.map(n => (
          <div
            key={n._id}
            className={`card !py-4 flex items-start justify-between gap-4 ${!n.is_read ? 'border-l-2 border-l-brand-500' : ''}`}
          >
            <div className="flex items-start gap-3 flex-1 min-w-0">
              <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${n.is_read ? 'bg-slate-600' : 'bg-brand-500'}`} />
              <div className="flex-1 min-w-0">
                <p className={`text-sm ${n.is_read ? 'text-slate-400' : 'text-slate-200'}`}>{n.message}</p>
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              <button
                onClick={() => openReply(n)}
                className="text-slate-500 hover:text-brand-400 p-1 rounded transition-colors"
                title="Reply"
              >
                <Reply size={14} />
              </button>
        ...
        footer={
          <>
            <button
              ...
              onClick={() => { setModal(false); setForm({ user_ids: [], message: '', type: 'general' }) }}
              disabled={sending}
            >
              Cancel
            </button>
          ...
          <FormField label="Recipients (leave empty = all users)">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-slate-500">
                {form.user_ids.length === 0
                  ...
                  : `${form.user_ids.length} of ${users.length} selected`}
              </span>
              <button
                type="button"
                onClick={selectAll}
            ...
            <div className="max-h-44 overflow-y-auto space-y-1 border border-white/5 rounded-xl p-2 bg-surface-200">
              {users.map(u => (
                <label
                  key={u._id}
                  className="flex items-center gap-3 px-3 py-1.5 rounded-lg hover:bg-white/5 cursor-pointer"
                >
                  <input
                    type="checkbox"
      ...
      <Modal
        ...
        footer={
          <>
            <button
              className="btn-secondary"
              onClick={() => { setReplyModal(false); setReplyTarget(null) }}
              disabled={replying}
            >
              Cancel
            </button>
        ...
        <div className="space-y-4">
          ...
          <FormField label="Type">
            <SelectInput
              ...
              onChange={v => setReplyForm(f => ({ ...f, type: v }))}
              options={TYPES.map(t => ({ value: t, label: t }))}
            />
          </FormField>
        </div>
