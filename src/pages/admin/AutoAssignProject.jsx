...
export default function AutoAssignProject() {
  ...
  useEffect(() => {
    axiosInstance.get("/users?role=manager").then((r) => setManagers(r.data.data || [])).catch(console.error);
  }, []);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {msg.text && (
        <div className={`rounded-lg px-4 py-3 text-sm flex justify-between items-start ${
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Auto-Assign Toggle */}
        <div className={`rounded-xl border-2 p-5 transition-colors ${autoAssign ? "border-blue-400 bg-blue-50" : "border-gray-200 bg-white"}`}>
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-gray-800">🤖 Smart Auto-Assignment</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField label="Start Date *">
              <input type="date" required value={project.start_date}
                onChange={(e) => updateProject("start_date", e.target.value)} className={INPUT} />
            </FormField>

            <FormField label="End Date *">
              <input type="date" required value={project.end_date}
                onChange={(e) => updateProject("end_date", e.target.value)} className={INPUT} />
            </FormField>
          </div>
        </Section>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-800">Assignments & Tasks</h2>
          ...
          {assignments.map((asgn, aIdx) => (
            <div key={asgn.id} className="border rounded-xl p-5 bg-white space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-gray-700">Assignment {aIdx + 1}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField label="Start Date *">
                  <input type="date" required value={asgn.start_date}
                    onChange={(e) => updateAssignment(aIdx, "start_date", e.target.value)} className={INPUT} />
                </FormField>
                <FormField label="End Date *">
                  <input type="date" required value={asgn.end_date}
                    onChange={(e) => updateAssignment(aIdx, "end_date", e.target.value)} className={INPUT} />
                </FormField>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button type="button" onClick={() => addTask(aIdx)}
                    className="text-xs px-2.5 py-1 border border-blue-400 text-blue-600 rounded hover:bg-blue-50">
                    + Add Task
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  {asgn.tasks.map((task, tIdx) => (
                    <div key={task.id} className="bg-gray-50 rounded-lg p-3 border space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-gray-500">Task {tIdx + 1}</span>
                        {asgn.tasks.length > 1 && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <FormField label="Required Role">
                          <input value={task.required_role}
                            ...
                            className={INPUT} />
                          {autoAssign && (
                            <p className="text-xs text-blue-500 mt-0.5">Used for auto-matching</p>
                          )}
                        </FormField>
                        <FormField label="Priority">
                        ...
                        <FormField label="Requires Admin Permission">
                          <div className="flex items-center gap-2 pt-2">
                            ...
                            <label htmlFor={`perm-${aIdx}-${tIdx}`} className="text-sm text-gray-600">
